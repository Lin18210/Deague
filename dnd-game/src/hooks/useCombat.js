...
export function useCombat(character, updateCharacter, addLog, onEndCombat) {
  ...
  const playerAttack = useCallback(() => {
    ...
    if (attackRoll.isCrit) {
      ...
      if (newHp <= 0) {
        ...
        setProcessing(false);
        setTimeout(() => onEndCombat(true), 500);
        return;
      }
    ...
    if (combatState.enemy.hp > 0 && attackRoll.total < combatState.enemy.ac) {
      setTimeout(() => enemyTurn(), 800);
    }
  }, [combatState, processing, character, strMod, profBonus, weapon, addCombatLog, addLog, onEndCombat]);
  ...
  const enemyTurn = useCallback(() => {
    setCombatState((prev) => {
      ...
      if (attackRoll.isCrit) {
        ...
        const newHp = character.health - totalDamage;
        if (newHp <= 0) {
          const deathEntry = `💀 ${character.name} has fallen in battle!`;
          addCombatLog(logEntry);
          addCombatLog(deathEntry);
          addLog(deathEntry);
          setDiceResult(attackRoll);
          setTimeout(() => onEndCombat(false), 500);
          return { ...prev, turn: 'player', round: prev.round + 1, log: [...prev.log, logEntry, deathEntry] };
        }

        updateCharacter({ health: newHp });
        addLog(`❤️ ${character.name} HP: ${newHp}/${character.maxHealth}`);
  ...
  }, [character, updateCharacter, addLog, onEndCombat]);
  ...
  const playerCastSpell = useCallback((spell) => {
    ...
    const attackRoll = rollD20(spell.attackMod);
    let logEntry;

    if (attackRoll.isCrit) {
      const match = spell.damage.match(/^(\d+)d(\d+)/);
      const doubled = match ? `${parseInt(match[1]) * 2}d${match[2]}` : spell.damage;
    ...
    if (attackRoll.isFumble) {
      ...
      const damage = rollDice(spell.damage);
      logEntry = `✨ ${character.name} casts ${spell.name}: rolled ${attackRoll.roll}+${spell.attackMod}=${attackRoll.total} vs AC ${combatState.enemy.ac} — HIT! ${damage.rolls.join('+')}=${damage.total} damage. Mana: ${character.mana - spell.manaCost}/${character.maxMana}`;
      ...
      logEntry = `✨ ${character.name} casts ${spell.name}: rolled ${attackRoll.roll}+${spell.attackMod}=${attackRoll.total} vs AC ${combatState.enemy.ac} — MISS! Mana: ${character.mana - spell.manaCost}/${character.maxMana}`;
    }
  ...
  }, [combatState, processing, character, updateCharacter, addCombatLog, addLog, onEndCombat]);

  const usePotion = useCallback(() => {
    if (!combatState || combatState.turn !== 'player' || processing) return;

    const healAmount = rollDice('2d4+2').total;
    const newHp = Math.min(character.health + healAmount, character.maxHealth);
    updateCharacter({ health: newHp });
    const logEntry = `🧪 ${character.name} drinks a healing potion: +${healAmount} HP (${newHp}/${character.maxHealth})`;
    addCombatLog(logEntry);
    addLog(logEntry);

    setCombatState((prev) => ({ ...prev, turn: 'enemy' }));
    setTimeout(() => enemyTurn(), 800);
  }, [combatState, processing, character, updateCharacter, addCombatLog, addLog]);
  ...
  const flee = useCallback(() => {
    ...
    if (escapeRoll.total >= dc) {
      ...
      setTimeout(() => onEndCombat(false), 300);
    } else {
      addCombatLog(`🚫 ${character.name} tries to flee but fails! (Rolled ${escapeRoll.total} vs DC ${dc})`);
      setCombatState((prev) => ({ ...prev, turn: 'enemy' }));
      setTimeout(() => enemyTurn(), 800);
    }
  }, [combatState, processing, character, dexMod, addCombatLog, addLog, onEndCombat]);
